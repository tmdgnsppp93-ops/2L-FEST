# J–V 곡선 비교 (GEDOS vs Solcore Quasi-3D) — FDM/FEM 관점 설명

> 첨부 그림: `GEDOS_vs_Solcore_quasi3D.png` (4패널: (A) 금속 마스크 / (B) J–V overlay / (C) 지표 비교 / (D) 결론)

![figure](GEDOS_vs_Solcore_quasi3D.png)
...
| 솔버 | 격자 방식 | 특징 |
|---|---|---|
| **GEDOS** | **FEM** (비정형 삼각 메시) | 핑거·버스바 **경계를 메시 선에 정확히 맞춤**. 국소 세분화 가능. |
...
빨강(GEDOS / FEM)과 파랑(Solcore / FDM) 두 곡선은 **모양이 사실상 같고 무릎(Voc) 위치도 일치**합니다. 차이는 파랑이 전류축에서 일정하게 약간 아래로 깔리는 것뿐입니다.

---

## 3. 차이값 (그림 C)

| 지표 | GEDOS | Solcore | 절대차 | 상대차 |
|---|---|---|---|---|
...
- **FEM (GEDOS)**: 비정형 메시가 50 µm 핑거 경계를 메시 선에 정확히 올립니다 → **그늘 면적이 기하학적으로 정확**(광학 그늘 6.1% ≈ Jsc 손실 6.7%).
...
## 5. 결론
- **핵심 물리(Voc·FF)는 FEM·FDM 두 독립 방법에서 0.6% 이내로 일치 → GEDOS 검증 완료.**
- **Jsc 차이는 FDM 정규격자의 전극 해상 한계**이며, **경계 적합 FEM을 사용하는 GEDOS(및 Griddler)가 전극·그리드 시뮬레이션에서 본질적으로 더 정확**합니다.
