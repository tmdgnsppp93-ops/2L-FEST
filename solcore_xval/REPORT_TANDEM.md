# 페로브스카이트/Si 탠덤 — GEDOS 계산 및 Solcore Quasi-3D 비교

![tandem figure](GEDOS_tandem_vs_Solcore.png)

> 한 줄 요약: **단일셀은 두 솔버가 정량적으로 일치(교차검증 완료)했고, 탠덤은 GEDOS(FEM)만 계산에 성공**했습니다. Solcore Quasi-3D는 실질적으로 단일접합 전용 도구이기 때문입니다.
...
## 2. GEDOS 탠덤 계산 결과 (그림 A) — 성공

GEDOS는 페로브스카이트/Si 2T(2-terminal, 직렬) 탠덤을 정상적으로 계산합니다:
...
- **탠덤**: GEDOS만 강건하게 계산. GEDOS의 솔버는 이 방정식 전용으로 만든 **FEM + Newton(연속법/감쇠)** 이라 stiff한 탠덤도 안정적으로 풉니다.

---

## 5. 결론
- **단일셀 비교**로 GEDOS의 핵심 물리(다이오드 + 횡전도)는 공개 표준 솔버와 **교차검증 완료**.
- **탠덤은 GEDOS(FEM-Newton)만 계산 가능** — Solcore Quasi-3D(SPICE/FDM)는 stiff한 페로브스카이트/Si 직렬 스택에서 수렴하지 못하며, 이는 Solcore 자체 예제도 단일접합만 시뮬레이션한다는 점에서 확인됩니다.
- 즉 **다중접합(탠덤) 시뮬레이션에서는 FEM 기반 전용 솔버(GEDOS)가 명확히 우위**입니다.
