...
"""Step (3b): GEDOS (FEM) vs Solcore Quasi-3D (SPICE) on the SAME single cell.

Drives the real GEDOS single-cell solver, rasterizes its exact metal grid into
...
W, H = GEO.W, GEO.H
print(f"GEDOS cell {W*10:.1f}x{H*10:.1f} mm, n_f={GEO.n_f}, n_b={GEO.n_b}, "
...
# /cf accounts for GEDOS's finger cross-section shape factor (area = w*h*cf),
...
# ---------------------------------------------------------------- report
# unit notes: GEDOS iv -> Jsc [mA/cm2], FF [%], Pmpp [mW/cm2].
#             Solcore iv_metrics -> Jsc [A/cm2], FF [frac], Pmax [W/cm2].
print("\n================  GEDOS (FEM)  vs  Solcore Quasi-3D (SPICE)  ================")
print(f"{'metric':12}{'GEDOS':>12}{'Solcore':>12}{'diff':>12}{'rel %':>10}")
...
plt.plot(Vf, Jf, "o-", label="GEDOS (FEM)", color="#C0392B", ms=4)   # Jf already mA/cm2
