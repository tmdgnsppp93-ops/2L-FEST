...
already handled by the current code:
  * v28.16 "Method B" makes the oVint DOF hold Vbot directly (V_int = Vbot + Vr
    ...
    is nonsingular by construction (2L_FEST_...:4615-4628, 4769-4774).
  * The rear-metal pad nodes ARE already Dirichlet-pinned to 0V
    (2L_FEST_...:4591, 4687-4688, 4760-4766).
...
def test_junction_bf_convergence(fest, make_bifacial, monkeypatch, rs_j, vb_frac):
...
def test_vb0_solution_accurate(fest, make_bifacial, monkeypatch):
...
def test_extreme_consistency(fest, make_bifacial, monkeypatch):
...
def test_junction_bf_nonsingular(fest, make_bifacial, monkeypatch):
