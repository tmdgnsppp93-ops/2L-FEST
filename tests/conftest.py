...
"""Headless test infrastructure for GEDOS (Phase 0).

Loads the single-file GUI application ``GEDOS.py`` as a
plain module named ``gedos`` WITHOUT a display or a real Tk/CustomTkinter
install, so the physics core (CellGeometry / generate_mesh / classify_nodes /
GEDOSSolver) can be unit-tested.
...
Strategy (order matters, all done before importing the app):
  ...
  2. Inject mock modules for tkinter (+ filedialog/messagebox), customtkinter,
     ...
     be a *real* subclassable class (``class GEDOSApp(ctk.CTk)`` runs at
     import); everything else is a permissive MagicMock.
  3. importlib-load the app file as ``gedos`` (this builds the default 9x9mm
...
class _DummyCTk:
    ...
    ``class GEDOSApp(ctk.CTk)`` executes at import time and needs a genuine
...
def _load_gedos():
    _install_gui_mocks()
    spec = importlib.util.spec_from_file_location("gedos", _APP_PATH)
    module = importlib.util.module_from_spec(spec)
    sys.modules["gedos"] = module
    try:
...
@pytest.fixture(scope="session")
def gedos():
    """The GEDOS application loaded headlessly as a module."""
    return _load_gedos()


def _build_solver(gedos, geo):
    pts, tri = gedos.generate_mesh(geo, axis_segments_override=36)
...
def _mono_geo(gedos):
    return gedos.CellGeometry(
        cell_w=2.0, cell_h=2.0,
        front=gedos.GridDesign(n_fingers=8, n_busbars=1,
                              w_finger=50e-4, w_busbar=600e-4),
    )


def _bifacial_geo(gedos):
    return gedos.CellGeometry(
        cell_w=2.0, cell_h=2.0,
        front=gedos.GridDesign(n_fingers=8, n_busbars=1,
                              w_finger=50e-4, w_busbar=600e-4),
        rear=gedos.GridDesign(n_fingers=6, n_busbars=1,
...
def mono(gedos):
    ...
    return _build_solver(gedos, _mono_geo(gedos))


@pytest.fixture(scope="session")
def bifacial(gedos):
    """Small bifacial cell: same front + patterned rear (6F+1BB)."""
    return _build_solver(gedos, _bifacial_geo(gedos))
...
def make_mono(gedos):
    ...
    """
    return lambda: _build_solver(gedos, _mono_geo(gedos))


@pytest.fixture
def make_bifacial(gedos):
    """Factory: build a FRESH bifacial solver (no warm-start history)."""
    return lambda: _build_solver(gedos, _bifacial_geo(gedos))
