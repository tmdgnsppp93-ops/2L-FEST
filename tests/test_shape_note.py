...
def test_dome_cf_is_pi_over_4(gedos):
    """반타원 단면적 (π/4)wh ÷ 외접 사각형 wh = π/4. 화면의 0.785 출처."""
    assert gedos.SHAPE_CF_DOME == math.pi / 4.0
    assert round(gedos.SHAPE_CF_DOME, 4) == 0.7854
    assert gedos.SHAPE_CF_RECT == 1.0


def test_recapture_slope_threshold(gedos):
    """유리 n=1.5: θc=41.81°, 필요한 표면 기울기는 그 절반인 20.9°."""
    a = gedos.recapture_slope_threshold_deg(1.5)
    assert abs(a - math.degrees(math.asin(1 / 1.5)) / 2.0) < 1e-12
    assert abs(a - 20.905) < 0.01


def test_recapture_fraction_matches_closed_form(gedos):
    ...
    for w, h in [(50.0, 10.0), (65.0, 8.5), (40.0, 20.0), (120.0, 5.0)]:
        ...
        got = gedos.dome_recapture_width_fraction(w, h)
        assert abs(got - expect) < 1e-12, (w, h, got, expect)


def test_default_before_finger_is_31_percent(gedos):
    """BEFORE 기본 단면 50×10 µm에서 화면에 찍히는 값이 31%인지 고정."""
    frac = gedos.dome_recapture_width_fraction(50.0, 10.0)
    assert abs(frac - 0.3094) < 5e-4
    assert f"{frac * 100:.0f}%" == "31%"


def test_flat_top_recaptures_nothing(gedos):
    """완전 평탄면은 기울기 0 → 정반사가 수직으로 되돌아 나가 재포획 0%."""
    assert gedos.dome_recapture_width_fraction(50.0, 0.0) == 0.0
    assert gedos.dome_recapture_width_fraction(0.0, 10.0) == 0.0
    assert gedos.dome_recapture_width_fraction(-1.0, 10.0) == 0.0


def test_recapture_is_monotonic_in_aspect_ratio(gedos):
    """가파를수록(h/w↑) 조건을 만족하는 폭이 넓어진다 — 평탄화=회수 감소."""
    prev = -1.0
    for h in (1.0, 2.5, 5.0, 10.0, 20.0, 40.0):
        cur = gedos.dome_recapture_width_fraction(50.0, h)
        assert cur > prev, h
        prev = cur
    assert 0.0 <= prev <= 1.0


def test_recovery_default_falls_back_without_front_electrode(gedos):
    """지연 임포트가 실패해도 문서화된 0.25로 떨어진다(엔진 독립성 유지)."""
    assert gedos._busbar_recovery_default() == 0.25


def test_note_header_fits_the_card(gedos):
...
def test_helpers_stay_gui_only(gedos):
    ...
    src = open(os.path.join(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))), "GEDOS.py"), encoding="utf-8").read()
    ...
    solver_body = src.split("class GEDOSSolver")[1].split(
