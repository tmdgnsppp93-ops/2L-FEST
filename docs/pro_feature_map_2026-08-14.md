...
| 6 | 비균일 공간 분포 입력 — txt/이미지에서 2D 맵 로드 | **부분구현 (결함 — 사용 보류)** | 로더 `load_spatial_map_txt` · GUI `_open_spatial_maps`는 완성. **엔진 배선은 소비 7곳 중 3곳만** — `2L_FEST.py:4932`(인라인) · `:6431`(single) · `:6717`(cell_current). 누락 `:5224` · `:5595` · `:5914` · `:6191` | §#6 상세 · `docs/spatial_map_convention.md` §6 (2026-08-18 2차 개정) |
...
> 2026-08-18 **2차** 개정 반영. 집계가 원 감사(2026-08-14)와 **같은 값으로 되돌아왔다** —
> #6이 부분구현 → 구현 → 부분구현(결함)을 거쳤기 때문이다. 다만 내용은 다르다:
> 원 감사의 "부분구현"은 *입력 경로가 없다*였고, 지금의 "부분구현"은
> *입력 경로는 있으나 엔진 배선이 분기 4곳에서 누락*이다(§#6 상세).
> 규약 일치(Griddler 대조)는 여전히 **미확인**이다.
