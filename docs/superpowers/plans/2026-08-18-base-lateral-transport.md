...
### 단위 2 — `Rs_base` 신설 + `_Kr` 병렬 합성 ✅ 완료 (2026-08-18, v28.59)

> **결과: `tests/test_base_lateral.py` 29 → 53 passed. 프로덕션 ~35줄.**
> 비트 핀 2건 strict 통과 — **캐시 해시 변경이 off 경로를 건드리지 않았다.**
>
> #### 캐시 해시 — 박사님이 지적하신 지점
>
> *"None일 때 해시 튜플에 뭘 넣을지가 off 경로 비트 동일을 결정한다."* 그대로다.
> **off는 상수 `0`**이며, 그것이 성립하는 논리는 공간 분포의 `_sm_tag`와 같다:
>
> | | 공간 분포 (v28.56) | 벌크 (v28.59) |
> |---|---|---|
> | off 값 | `0` | `0` |
> | 왜 `0`이 안전한가 | 유효한 `content_key()`는 튜플이라 `0`이 될 수 없다 | **`Rs_base ≤ 0`을 거부**하므로 유효값이 `0`이 될 수 없다 |
> | 왜 상수여야 하는가 | off 슬롯이 상수라야 hit/miss 판정이 예전 필드들만으로 결정된다 | 동일 |
>
> **테스트 3건으로 못 박았다:**
>
> - `test_base_touches_exactly_one_hash_slot_and_off_is_a_constant` — `Rs_base`가
>   해시에서 **정확히 한 슬롯만** 건드리고 off일 때 그 슬롯이 `0`이다. 슬롯
>   번호를 하드코딩하지 않고 **diff로 실측**한다(나중에 필드가 추가돼도 안 깨진다)
> - `test_off_hash_slot_stays_constant_across_other_parameter_changes` — 다른
>   파라미터를 8가지로 바꿔도 off 슬롯은 계속 `0`
> - `test_zero_is_rejected_so_the_off_sentinel_cannot_collide` — 센티넬 충돌 불가
>
> #### 값 제약은 해시·조기 반환보다 **먼저**
>
> 검증이 조기 반환 뒤에 있으면 **같은 파라미터 두 번째 호출에서 잘못된 값이 캐시
> 적중으로 통과한다.** v28.57 로더가 *"값 제약은 읽는 시점에"*로 정리한 것과 같은
> 이유다. `test_base_validation_runs_before_the_cache_early_return`이 지킨다.
>
> #### 테스트를 쓰다 걸린 것 2건
>
> 1. **`test_base_off_is_bit_identical`을 "같은 솔버로 두 번 풀기"로 썼다가
>    실패했다.** 두 번째 solve는 warm-start를 쓰므로 답이 미세하게 다르다 —
>    벌크와 무관한 기존 성질이다. 조립된 `_Kr`을 `assemble_K(Rs_rear_tco)`와 직접
>    비교하는 방식으로 바꿨다(산술적 근거). 비트 동일의 최종 확인은 **변경 전 값을
>    들고 있는 비트 핀**이 한다.
> 2. `"500"`은 `float()`이 받으므로 비숫자가 아니다. `_parse_gui_float`가 GUI
>    입력을 문자열로 받는 것과 같은 관용이라 그대로 두고, 테스트를 진짜 비숫자
>    (`"abc"` / 리스트 / dict / object)로 고쳤다.
>
> #### 단위 0의 기준선 테스트를 뒤집었다
>
> `test_no_base_lateral_parameter_exists_yet` →
> `test_base_parameter_surface_is_exactly_one_field`. 이제 **`Gv_base`가 없어야
> 한다**는 쪽이 더 중요하다 — 생기면 토폴로지가 α로 돌아갔다는 뜻이고,
> `Rs_vert_bot` 이중 계산 거부 판정을 되살려야 한다(규약 §5). 실패 메시지에
> 그렇게 적었다.
>
> #### ⚠ 이 단위 종료 시점의 공백 → **단위 3에서 해소됨 (v28.60)**
>
> v28.59 종료 시점에 **`full_area` + `Rs_base`가 조용히 무효**였다 — 오류도 경고도
> 없이 아무 효과가 없는, 이 계획이 없애려는 바로 그 실패 형태다. 커밋을 나눈
> 대가이므로 **같은 작업 묶음에서 단위 3을 이어 끝냈다.** GUI 배선이 단위 5에
> 있어 그 사이 사용자에게 노출되지는 않았다.
...
### 단위 3 — `full_area` 거부 게이트 ✅ 완료 (2026-08-18, v28.60)

> **결과: `tests/test_base_lateral.py` 53 → 68 passed. 프로덕션 ~25줄.**
> 비트 핀 2건 strict 통과.
>
> #### 게이트 위치 — `_build` 한 곳
>
> 진입점이 `solve_tandem`(`:4787`) · 연속법 램프(`:4817`·`:4824`) ·
> `solve_single`(`:6343`) · `:7436`으로 흩어져 있지만 **전부 `_build`를 거친다.**
> 한 곳이면 충분하다.
>
> 위치는 값 검증 바로 뒤, **해시·조립보다 먼저**다. 뒤에 두면 `_cache_hash`만
> 갱신되고 평면은 옛것인 상태가 남는다 —
> `test_rejection_leaves_the_solver_usable`이 지킨다.
>
> #### 조건을 한 곳으로 모았다 (리팩터)
>
> 후면 조립 분기가 `rear_mode in ('bifacial','patterned') and self.isrm is not
> None`인데, **게이트가 이 조건을 따로 쓰면 나중에 한쪽만 바뀌어 어긋난다** —
> 그러면 *"게이트는 통과했는데 이상적 접촉 평면이 조립되는"* 조용한 무효가
> 되살아난다. `_rear_is_a_plane` boolean 하나를 위에서 계산해 **양쪽이 같은
> 조건을 보게** 했다.
>
> `test_gate_and_rear_assembly_share_one_condition`이 이 성질을 감시한다 —
> 거부되는 설정에서는 후면이 `assemble_K(0.001)`이고, 통과하는 설정에서는
> 아니어야 한다.
>
> #### 지원 기준은 `rear_mode` 하나
>
> `test_base_gate_follows_the_rear_plane_not_the_phase`가 **단위 0의 6분기 표
> 전체**를 돌면서 `Nrm > 0`이면 통과 / 아니면 거부임을 확인한다. Phase A/B도,
> tandem/single도 기준이 아니다.
>
> 초안(α)의 *"7곳 중 2곳 지원 + 5곳 거부"*가 β에서 **"후면 평면이 실제인 곳은
> 전부 지원"**으로 단순해졌다는 것의 검증이다.
>
> #### 오류 메시지
>
> ```
> 벌크 횡전도(Rs_base=500.0 Ω/sq)는 rear_mode='full_area'에서 표현할 수 없다.
> full_area는 후면을 이상적 접촉(0.001 Ω/sq, V_rear ≡ 0)으로 두므로 후면 평면의
> 면전도를 바꿔도 결과가 수학적으로 변하지 않는다 — 값을 받아도 조용히 아무
> 효과가 없다. rear_mode를 'bifacial'/'patterned'로 두거나 Rs_base=None으로 끌 것
> (docs/base_lateral_convention.md §3-4).
> ```
>
> **현재 `rear_mode`와 사용자가 넣은 값을 그대로** 적고, **왜** 안 되는지와
> **어떻게 하면** 되는지를 함께 준다. 거부만 하고 길을 안 알려주면 사용자는 무엇을
> 고쳐야 하는지 모른다 — v28.57 로더가 파일명·행·열·값을 적은 것과 같은 이유다.
> `test_rejection_message_names_the_actual_rear_mode`가 고정한다.
