...
> #### ⚠ 이 단위 종료 시점의 알려진 공백
>
> **`full_area` + `Rs_base`는 아직 조용히 무효다.** 오류도 경고도 없이 아무 효과가
> 없다 — 이 계획이 없애려는 바로 그 실패 형태다. **단위 3(거부 게이트)을 같은
> 작업 묶음에서 반드시 끝낼 것.** 여기서 멈추면 저장소가 깨지지는 않지만
> (`Rs_base`는 새 옵트인 입력이고 기본값이 off다) 사용자에게 노출하면 안 된다 —
> GUI 배선이 단위 5에 있는 것이 그래서 다행이다.
