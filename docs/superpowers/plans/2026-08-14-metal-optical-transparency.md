...
- ~~Task 2 `test_transparency_does_not_touch_contact`의 `l1["Pc"] == l0["Pc"]`는 `==` 비교다. `_Gc`는 물리 폭만 쓰므로 성립해야 한다.~~
  **⚠ 2026-08-14 실행 중 정정 — 이 단언 자체가 틀렸다.**

  구현 후 실제로 깨졌고, 원인은 배선 누수가 아니라 **단언 대상을 잘못 고른 것**이었다. `Pc = Σ(Ve−Vm)²·Gc`는 **저항이 아니라 전력**이다. `Gc`가 그대로여도 T가 발전량을 늘리면 전류가 늘어 접촉을 가로지르는 전압강하가 커지고, `Pc`는 물리적으로 **당연히 오른다.**

  측정으로 확인한 사실 (20 mm 소셀, T 0→0.4):

  | 양 | 결과 |
  |---|---|
  | `_Gc` (접촉 컨덕턴스) | **비트 동일** (max\|diff\| = 0.0) |
  | `_Km` (금속 저항망) | **비트 동일** (nnz diff = 0) |
  | `_Ke` (emitter 시트) | **비트 동일** |
  | `metal_frac` | **비트 동일** |
  | `illum_frac` 합 | 2776.82 → 2822.34 (**×1.0164**) |
  | `Pc` | 0.10745 → 0.11102 (**×1.0332**) |

  `1.0164² = 1.0331` — `Pc` 증가가 발전량비의 제곱과 소수점 넷째 자리까지 일치한다. 저항 소산이 전류²에 비례하므로 정확히 예상되는 값이다. 즉 **물리/광학 분리는 처음부터 올바르게 작동했다.**

  **교훈: 불변이어야 하는 것은 모델(`_Gc`/`_Km`/`metal_frac`)이지 소산 전력이 아니다.** 소산 전력은 해(solution)의 함수라 입력이 바뀌면 당연히 따라 움직인다.

  현재 테스트 구성:
  - `test_transparency_does_not_touch_contact` — 모델 4종이 비트 동일함을 `==`로 확인 (완화 아님, 대상 교체이며 검사 항목은 오히려 늘었다)
  - `test_transparency_effects_stay_in_optical_path` — 결과 수준. `P_shade` 감소, `Eff` 증가, `Pc`/`Pf_*`의 증가가 발전량비 제곱의 2배 이내(느슨한 상한)임을 확인해 "딴 경로로 새지 않았다"를 방어
