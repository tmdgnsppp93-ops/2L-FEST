...
### 우선순위 3 — Capacitive effects (I-V 스윕 과도 효과) — 📋 **계획서 작성됨 (2026-08-19)**

> **계획: `docs/superpowers/plans/2026-08-19-capacitive-effects.md`**
> 코드는 한 줄도 쓰지 않았다. 착수 전에 그 문서를 읽을 것 — 아래 "작업 범위"보다
> 정확하다. 특히 두 가지가 아래 기술과 다르다.
>
> 1. **아래 3번("잔차·야코비안에 dV/dt 항 추가")은 새 미지수를 만들지 않는다.**
>    음함수 차분이면 대각 기여 + 잔차 상수항뿐이다 — 미지 벡터 레이아웃·희소성
>    패턴이 불변이다. base lateral의 β 토폴로지와 같은 결론이되 이유가 다르다
>    (그쪽은 `assemble_K`의 선형성, 이쪽은 시간 이산화 방식의 선택).
>    ⚠ 이 판정은 **시간 적분 방식 확정이 전제**다.
> 2. **아래 1번("스윕 루프 구조가 바뀐다")은 더 강하게 말해야 한다** —
>    `calc_iv`(`:7154`)는 스윕이 아니다. 이력 의존과 양립 불가한 요소가 넷이다:
>    전압 키 `eval_cache`(`:7184-7198`) · 수렴 편의를 위한 고→저 순서(`:7208`) ·
>    비단조로 왕복하는 `_bisect_root`(`:7213`) · 나중에 점을 삽입하는 MPP 적응
>    리파인먼트. → 기존 경로를 고치지 말고 **`calc_iv_transient`를 신설**한다.
>
> 그리고 **dV/dt→0 극한은 비트 동일이 아니다**(아래 검증 조건 문장이 부정확하다).
> 과도 **off**에서 비트 동일이고, 과도 on + dV/dt→0은 정상상태 해로 **수렴**한다.
> 둘을 섞으면 테스트가 영원히 빨간불이 되거나 허용오차를 몰래 키우게 된다.
>
> **페로브스카이트 이온 이동 히스테리시스는 범위 밖이다** — PRO에 없는 물리이고
> 시간 상수가 3~6자리 다르다. 계획서 §0 참조.
>
> ⚠ **매뉴얼 PDF가 저장소에 없다.** §6.6 관련 in-repo 기록은
> `docs/griddler_feature_map.md:310`의 `| 6.6 | Transient 특성 | C | ? |` 한 줄뿐이고
> §1.2 Core Model 등가회로도 정리돼 있지 않다. 계획서는 [지시]/[repo]/[유도]를
> 표식으로 분리해 두었다 — **단위 0의 첫 작업이 매뉴얼 확인**이다.
