...
```
pytest -q -m "not slow"
423 passed, 2 deselected, 6 xfailed          # 2026-08-19 KIST PC, 1411 s
```

> **2026-08-19 갱신.** 위 수치는 KIST PC(원 캡처 PC)에서 v28.61 기준으로 실측한
> 것이며, 같은 날 집 데스크톱 결과(423 / 2 / 6)와 **정확히 일치**한다.
...
**착수 대상: 우선순위 3 — Capacitive effects, `단위 0`의 남은 절반(특성화 테스트).**

**우선순위 0은 단위 0·1·2 전부 완료됐다(v28.61 + 문서).** 우선순위 3 단위 0의
매뉴얼 확인도 2026-08-19 KIST PC에서 끝났다.

| 항목 | 값 |
|---|---|
| 계획서 | `docs/superpowers/plans/2026-08-19-capacitive-effects.md` — **§1-0(매뉴얼 판정)과 §6-0(범위 재산정)을 먼저 읽을 것** |
| 남은 것 | `BRANCH_CASES`를 import해 **미지 벡터 길이·희소성 패턴 불변**을 6분기 전부에서 고정 |
| 프로덕션 | **0줄** |
