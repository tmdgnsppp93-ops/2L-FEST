...
```
pytest -q -m "not slow"
470 passed, 2 deselected, 6 xfailed          # 2026-08-19 KIST PC, 1452 s
```

> **2026-08-19 갱신.** 위 수치는 KIST PC(원 캡처 PC)에서 v28.61 기준으로 실측했다.
> 이전 판의 `147 passed`는 v28.55 시점 값이다.
>
> | 시점 | 통과 | 비고 |
> |---|---:|---|
> | 집 데스크톱, 우선순위 0 단위 1 직후 | 423 | v28.61 |
> | KIST PC, 같은 트리 | **423** | 두 머신 **정확히 일치** |
> | + 우선순위 3 단위 0 (`test_transient_layout.py` 47건) | **470** | 프로덕션 0줄 |
>
> **비트 핀 2건이 strict로 통과했다** — xfailed 6건은 전부 아래 `test_junction_bf`의
> Vb=0 항목이고, 스택 불일치 xfail은 발동하지 않았다. 즉 우선순위 0 단위 2의
> **핀 확인이 원 캡처 PC에서 끝났다.**
...
> **2026-08-20 갱신 — 우선순위 3(capacitive effects)은 드롭 확정이다**(§3-제외).
> 위 문장은 당시 판단 기록으로 남긴다. 결과적으로 **솔버의 물리 경로를 바꾸는
> 미착수 항목은 남아 있지 않다** — 원 캡처 PC가 필요한 머지 대상도 없다.
...
**v28.64 후속**: 비활성 드롭다운을 지우고 **읽기 전용 상태 표시**로 바꿨다
(전면·후면 두 곳, 라벨 `Probe Point` / `프로브 점 방식`).

v28.54의 조치는 오해를 막았지만 **위젯 모양이 남긴 오해**가 하나 더 있었다 —
비활성 드롭다운은 "곧 활성화될 것"으로 읽힌다. 고를 수 없는 것을 선택 위젯
모양으로 두면 사용자는 값이 아니라 **시점**을 기다린다. 선택지가 하나뿐이면
그것은 선택이 아니라 상태이므로 상태 위젯(라벨)이 맞다.

`_extract_method_var` / `_rear_extract_method_var`(StringVar)는 **남겼다.**
지우면 `_apply_grid_design`이 getattr 폴백 경로로 갈아타는데, 그 폴백은 같은
값을 내지만 경로가 다르다 — 위젯을 지우는 변경에서 읽기 경로까지 함께 바꾸면
결과가 같은지 판정할 축이 둘이 된다. 표시 라벨과 var 초기값이 갈리지 않도록
둘 다 `EXTRACTION_METHOD_FIXED_LABEL` 모듈 상수에서 받는다.

- **솔버 배선은 여전히 없다.** `GridDesign.extraction_method`는 `probe_point`
  그대로이며, 이 변경은 계산 경로를 0줄 건드렸다.
...
### 2-6. 공간 분포 입력 인터페이스 (구 §3 우선순위 1) — 완료 ✅ **결함 해소 (2026-08-19, v28.61)**
...
>
> **핵심 산출물은 수정 자체가 아니라 재발 방지다** — 소스 검사
> (`INLINE_ASSEMBLY_CENSUS` 13함수 34줄 → **1함수 4줄**)와 런타임 헬퍼 호출 계수를
> **둘 다** 둔다. 소스 검사는 우회 가능하고 런타임 계수는 새 분기를 모르기 때문이다.
>
> 상세: `docs/spatial_map_convention.md` §6 ·
> `docs/pro_feature_map_2026-08-14.md` #6 (3차 개정: 부분구현 → **구현**) ·
> 세션 `docs/sessions/2026-08-19-spatial-branch-coverage-unit0.md` · `…-unit1.md`.
>
> ⚠ **보류 기간에 산출된 값은 신뢰할 수 없다** — v28.60 이전의 Phase B 결과에
> 맵을 걸었다면 다시 계산할 것.
>
> **아래 보류 블록과 "완료" 기록은 지우지 않는다** — 무엇이 왜 결함이었고 어떤
> 판정 함정이 있었는지가 남아야 같은 일이 반복되지 않는다.

> ## ⛔ [해소됨 — 2026-08-18 당시 기록] 이 항목은 "완료"이지만 **기능은 사용 보류**다
>
...
> 만든다.
>
> > ⚠ **뒤에 밝혀진 것**: 결함 분기는 4개가 아니라 **5개**(`_solve_single_bifacial`
> > 추가)이고 **단일셀도 영향을 받았다**. *"`J01_top_arr`를 스칼라로만 만든다"* 도
...
> 않는 값이다).
>
> > **이 판정 함정 경고는 해소 후에도 그대로 유효하다** — `ΔJ ≠ 0`은 고치기
> > 전에도 참이었으므로 **앞으로도 작동 근거로 쓰면 안 된다.**
>
...
| `40d4ea9` | **[우선순위 0]** 단위 0 — 분기 커버리지 특성화 97건 (프로덕션 0줄) |
| `49222fa` | **[우선순위 0]** v28.61 단위 1 — `_diode_node_arrays` 중앙화 + 13곳 배선 |
| (이번) | **[우선순위 0]** 단위 2 — 문서 갱신 (이 블록 포함) |
...
### 2-7. 공간 분포 5번째 대상 — `rsh` (션트 저항), v28.62 ✅

**보류 조건이 지켜진 뒤에 착수했다.** 계획서 §하지 않는 것이
*"지금 추가하면 같은 공백을 하나 더 복제한다. 이 계획이 끝난 뒤에 한다"* 고
적었고, 실제로 단위 2까지 끝난 뒤에 했다. 그때는 `_diode_node_arrays`가 이미
있어서 **복제할 공백이 없었다** — 우선순위 0 전에 추가했다면 결함 칸이 12개가
아니라 18개가 됐을 것이다.

| 항목 | 값 |
|---|---|
| 근거 | Griddler 매뉴얼 §3.1이 *"most cell parameters"* 에 shunt conductance 포함, GUI에도 비균일 진입 버튼. `Rsh`는 이미 노드 잔차(A 계층)에 있어 j01/j02와 **같은 계층** |
| 단위 0 | 6분기 × rsh 6칸을 `False`로 고정 + strict xfail 12건 (프로덕션 0줄) |
| 구현 | `_diode_node_arrays`가 `Rsh`/`Rshb`를 함께 반환. 소비 지점 **12함수 47줄** |
| 검증 | 12칸이 전부 XPASS → 표 갱신 → **116 passed · 0 xfailed** |
| 물리 변경 | **없음** — 맵 없으면 비트 동일. 비트 핀 2건 strict 통과 |
| 기록 | `docs/sessions/2026-08-19-spatial-shunt-target.md` · 규약 `docs/spatial_map_convention.md` §7 |

**⚠ 반드시 알아야 할 두 가지**

1. **방향이 반대다** — 맵이 션트 **저항**을 곱하므로 값이 크면 누설이 줄어
   셀이 좋아진다. `rc`와 같은 함정이라
   `SPATIAL_INVERTED_TARGETS = ("rc", "rsh")`를 **엔진에** 신설했고 GUI가 그
   목록을 보고 힌트를 앰버로 강조한다. 목록을 GUI에 두지 않은 이유:
   방향이 뒤집힌다는 것은 모델의 사실이지 화면의 사실이 아니다.
2. **Griddler와 곱셈 방향까지 반대다** — 그쪽은 컨덕턴스(S/cm²), 우리는
   저항(Ω·cm²)이라 배율 파일이 서로 **역수**여야 한다. **Griddler 맵을 그대로
   넣으면 누설이 심한 자리가 가장 깨끗한 자리로 계산된다.** 규약 §7.

**비트 동일 설계**: 맵이 없으면 헬퍼가 `dp.Rsh_*` **스칼라 그 객체**를 그대로
돌려준다. 소비 지점이 전부 `V / Rsh`(나눗셈)이라 식이 v28.61과 문자 그대로 같다.
⚠ 컨덕턴스 `1/Rsh`를 돌려주고 `V * Gsh`로 바꾸면 **안 된다** —
`V / R`과 `V * (1/R)`은 마지막 비트가 다르다. 독스트링·changelog·테스트 세 곳에
금지 사유를 적었다.
...
>
> **2026-08-20 (3차): 우선순위 3(Capacitive effects)은 드롭 확정이다.** 구현하지
> 않는다 — **§3-제외 표로 옮겼고 근거 2건은 거기에 있다.** 계획서
> `docs/superpowers/plans/2026-08-19-capacitive-effects.md`는 **삭제하지 않는다**:
> 드롭 판정의 근거(매뉴얼 원문 대조)가 그 안에 있다. 번호는 여기서도 재조정하지
> 않는다. → **구현 확정 3건은 이로써 전부 종결됐다**(1·2 완료, 3 드롭).

---

## ▶ 다음 착수 지점 (2026-08-19 KIST PC 기준) — ⛔ **무효 (2026-08-20)**

> ⛔ **이 블록의 착수 대상은 더 이상 유효하지 않다.** 우선순위 3(Capacitive
> effects)이 **드롭 확정**됐다(§3-제외, 2026-08-20 랩미팅). 아래 내용은 **지우지
> 않는다** — 무엇을 어디까지 준비했는지가 남아야 한다. 다만 **단위 1 이후는
> 착수하지 말 것.**
>
> **단위 0의 산출물 `tests/test_transient_layout.py` 47건은 그대로 유지한다.**
> 그 파일이 고정하는 것은 **선형 시스템의 모양**(미지 벡터 레이아웃 · 희소성 패턴 ·
> 대각 슬롯의 구조적 존재)이고, 이는 **시간 항의 유무와 무관하게 성립한다.**
> 드롭됐다고 지우면 그 불변식만 조용히 풀린다.

> ~~**여기서부터 이어서 하면 된다.**~~ — **취소됨.** 다음 착수 지점은 §3에서
> 다시 고를 것.

**착수 대상: ~~우선순위 3 — Capacitive effects, `단위 1`(소비 지점 census).~~ → 드롭 확정.**

**우선순위 0은 단위 0·1·2 전부 완료(v28.61 + 문서)이고, 우선순위 3 단위 0도
2026-08-19에 끝났다** — 매뉴얼 확인 + 특성화 테스트 양쪽 다.
그 뒤 **공간 분포 5번째 대상 `rsh`도 완료**됐다(v28.62 → §2-7). 보류 목록에
남아 있던 항목이라 §별건에서 내려왔다.

> ⚠ **우선순위 3 착수 전에 §2-7의 비트 동일 설계를 볼 것.**
> 시간 항도 `_diode_node_arrays`가 만드는 노드 배열과 같은 자리에 들어간다.
> `rsh`가 `V / Rsh`(나눗셈)를 유지해 비트를 지킨 것과 같은 판단이 필요하다 —
> *"곱셈이 빠르다"* 로 식의 형태를 바꾸면 핀이 깨진다.

| 항목 | 값 |
|---|---|
| 계획서 | `docs/superpowers/plans/2026-08-19-capacitive-effects.md` — **§1-0(매뉴얼 판정)과 §6-0(범위 재산정)을 먼저 읽을 것** |
| 다음 단위 | **단위 1 — 소비 지점 census.** 시간 항이 들어갈 자리(잔차 5분기 + 인라인 2)와 `calc_iv` 계열을 AST로 census. 우선순위 0의 `INLINE_ASSEMBLY_CENSUS`와 같은 형태 |
| 프로덕션 | **0줄** (단위 1까지) |
| 머신 | **아무 데서나 된다** — 매뉴얼도 원 캡처 PC도 필요 없다 |
| 현재 버전 | **v28.61** |

> ⛔ **구현(단위 2 이후) 착수 전에 반드시 읽을 것**: 계획서 §6-0 (b)(c).
> 이 작업은 *"PRO 따라가기"* 가 **아니다.**

### 우선순위 3 단위 0 — 특성화 결과 (2026-08-19) ✅

`tests/test_transient_layout.py` **47건 / 27 s. 프로덕션 0줄.**
`BRANCH_CASES`를 **import**해서 쓴다(복제 금지 — 두 벌이 되면 갈린다).

**§2의 판정에 숨은 전제가 하나 있었고, 그것이 실측으로 확인됐다.**

> 대각에 `+C/Δt`를 더하는 것이 희소성을 바꾸지 않으려면 **대각 자리가 이미
> 구조적으로 존재해야 한다.** 하나라도 비면 시간 항이 `nnz`를 늘린다.

| 분기 | Ns | nnz | 대각 슬롯 |
|---|---:|---:|---:|
| Phase A / full_area | 12,980 | 51,380 | 12,980 |
| **Phase B / full_area** (기본) | 16,789 | 83,447 | 16,789 |
...
- `spatial_j01`(노드마다 다른 계수)을 걸어도 **패턴 동일**

> ⚠ *"지금 그렇다"이지 "항상 그렇다"가 아니다.* 조립을 *"0인 대각은 빼자"* 로
> 최적화하면 `test_every_diagonal_entry_is_structurally_present`가 먼저 깨진다.
> **그때 고칠 것은 테스트가 아니라 계획서 §2의 판정이다.**

계획서와 달라진 것: `indptr` 대신 **`(row, col)` 좌표 집합 sha256**을 썼다 —
`indptr`은 CSR/CSC 포맷에 따라 값이 달라져 물리와 무관하게 깨진다. 리터럴 핀
(`nnz`+해시)은 메시(Qhull) 의존이라 `bit_pin_gate`로 낮췄고, **구조 불변식 4종은
어느 머신에서도 strict**로 돈다.

### 우선순위 3 단위 0 — 매뉴얼 확인 결과 (2026-08-19, KIST PC) ✅

매뉴얼 PDF(`~/Desktop/이승훈/Griddler_and_PRO_manual.pdf`, v7.0 2023-09-15, 123쪽)를
읽어 계획서의 **[지시] 표식 15건을 전부 판정**했다. 판정표는 계획서 §1-0,
원문 요약은 `docs/griddler_feature_map.md` §1-1 · §2.8-a · §2.10 · §2.11-a에 있다
(**다른 머신에서도 검증 가능하도록 in-repo에 옮겼다**).

**세 줄 요약**

1. **§1.2 등가회로에 용량 소자가 없다.** 소자는 n=1 다이오드 · n=2 다이오드 ·
   광생성 전류원 · shunt 컨덕턴스가 전부다. 매뉴얼 전체에서 `capacitance`가
   소자를 가리켜 쓰인 곳이 **0건**이다.
2. **§6.6은 모델을 공개하지 않는다.** 본문이 문단 1 + 명령어 2 + 그림 1이고
   **수식이 한 줄도 없다.** 지목하는 원인도 용량이 아니라 *"high lifetime →
   ms 단위 정상상태 도달 시간"* 이다.
3. **§6.2가 결정적이다.** *"the Griddler solver is inherently for steady state"* 라고
   스스로 적고, 시간 의존성을 **base transport calculator 안에서 복소 벌크 수명**
   (`1/τ* = 1/τ + iω`)으로 근사한다. **FEM에는 소자가 추가되지 않는다.**

**그래서 무엇이 바뀌었나**

| # | 내용 |
|---|---|
| 1 | 이 작업은 **자체 규약**이다. *"PRO 등가"* 로 인용하면 안 된다 — 공간 분포 규약과 같은 처리 |
| 2 | **용량을 물성에서 유도하지 않는다.** 노드 용량을 직접 입력받는다. τ·SRV·두께에서 유도하려면 `pro_feature_map` #2(**구현 불필요 확정**) 계층이 필요하다 |
| 3 | 스윕 입력이 **(t, V) 2열 표 하나**로 정리된다. `SETSWEEPRATE`(등속)는 두 점짜리 표의 특수 케이스다 |
| 4 | **§6.2 AC 응답은 더 멀어졌다.** *"같은 용량 모델을 공유한다"* 는 예상이 틀렸다 |
| 5 | 계획서의 [repo] 전제 하나가 틀렸다 — `griddler_feature_map.md`에 §1.2 절이 **있었다.** 없던 것은 등가회로 소자 목록이다 |

**부수 발견 — 벌크 횡전도 문서의 수치 정정**

`docs/base_lateral_convention.md`가 세 곳에서 인용한
*"1 Sun 0.05 %p vs 3.6 Suns 0.11 %p"* 는 **두 값의 단위가 다르다.** 매뉴얼 p.49
원표에서 계산하면 1 Sun **+0.0099 %p**(상대 0.048 %), 3.6 Suns **+0.1056 %p**
(상대 0.508 %)다. 절대 %p로 통일하면 **약 10.6배**이며 인용된 2.2배가 아니다.
**결론은 바뀌지 않고 강해진다.** 정정 기록은 그 문서 머리와
`griddler_feature_map.md` §2.8-a에 남겼다.
...
- ~~**`Rsh` 5번째 대상** — 박사님 지시로 보류~~ → ✅ **v28.62에서 구현**(§2-7)
- **`pro_feature_map` #8 판정 갱신** — Base lateral transport가 v28.59·v28.60에서
  구현됐는데 그 문서는 아직 **미구현**으로 적혀 있다. 벌크 횡전도 계획서 단위 5의
  남은 항목이다
- **이미지 입력(jpg/tif/bmp)** — Pillow 의존성. 저작권 등록 이후

### 환경 — 이 항목은 해소됐다

이 아래 "미해결 환경 결함" 블록(`import numpy.testing` 무한 대기)은 **2026-08-19에
재현되지 않았다** (2.55 s / 4.14 s로 정상). 원인은 여전히 미규명이고 **머신 상태
변화라는 진단만 확인됐다.** 재발하면 그 블록에 추가 기록할 것.

또한 **집 데스크톱이 핀 스택임이 확인됐다** — §1-2 갱신 참조. 물리 변경의 비트 핀
검증을 KIST PC까지 가지 않고 거기서 할 수 있다.
단, `python -m pytest`(시스템)로 돌릴 것 — `.venv`는 스택이 다르다.

<details><summary>미해결 환경 결함 원문 (2026-08-18, 집 데스크톱) — 기록 보존</summary>
...
### 우선순위 0 — 공간 분포 맵 분기 커버리지 수정 — ✅ **완료 (2026-08-19, v28.61)**

> **단위 0 `40d4ea9` · 단위 1 `49222fa`(v28.61) · 단위 2(문서) 이번 커밋.**
> 아래 착수 시점 기술은 **그대로 둔다** — 실제가 이 예상과 어떻게 달랐는지가
> 기록으로 남아야 한다.
>
> **예상과 달랐던 점 네 가지** (전부 단위 0의 실측이 찾아냈다):
>
> | # | 계획서 | 실제 |
> |---|---|---|
> | 1 | 결함 분기 4개 | **5개** — `_solve_single_bifacial` 추가 |
> | 2 | 소비 지점 9곳 | **13개 함수 34줄** (배선 후 1함수 4줄) |
> | 3 | `J01_top_arr`가 스칼라 | **아니다.** `mf`가 배열이라 결함 분기에서도 `ndarray[N]` |
> | 4 | 결함 칸 9개 | **12개** |
>
> 결과: 판정 파일 **97 passed · 0 xfailed**, 비트 핀 2건 strict,
> Phase A 5조합 비트 동일. 상세는 §2-6.
...
### 우선순위 1 — 공간 분포 입력 인터페이스 (파일 로더 + GUI) — ✅ **완료 → §2-6** (결함은 우선순위 0에서 해소)

**2026-08-18 완료 (v28.56 ~ v28.58).** 상세·발견·남은 것은 **§2-6**에 있다.
**여기서 발견된 결함은 우선순위 0으로 분리해 2026-08-19에 해소됐다(v28.61).**
...
### 우선순위 3 — Capacitive effects (I-V 스윕 과도 효과) — ⛔ **드롭 확정 (2026-08-20)**

> ⛔ **구현하지 않는다.** 상태: 보류 → **드롭 확정**. → **§3-제외 표**(근거 2건).
> 아래 작업 범위·검증 조건 기술은 **당시 분석 기록으로 남긴다** — 지우지 않는다.
> 계획서도 삭제하지 않는다.
>
> 남는 산출물은 **단위 0의 `tests/test_transient_layout.py` 47건뿐이고, 유지한다**
> (선형 시스템의 모양을 고정하는 테스트라 시간 항과 무관하게 유효하다).
...
### 구현 불필요 확정 (박사님 판단, 2026-08-17 · 2026-08-20 추가)

아래 4건은 **구현하지 않는다.** 감사 보고서에 "미구현"으로 적혀 있어도 격차로
취급하지 말 것.

| 항목 | 상태 | 사유 |
|---|---|---|
| **RayFlare 광학 연동** | 구현 불필요 (2026-08-17) | 랩 내부에서 **Python-CROWM**을 쓴다 |
| **PC1D 에미터 계산 연동** | 구현 불필요 (2026-08-17) | 지시 범위 밖 |
| **Cell cross-sectional model** | 구현 불필요 (2026-08-17) | 지시 범위 밖. J01_pass/J01_metal은 역산이 아니라 직접 입력이 현 설계 |
| **Capacitive effects (I-V 스윕 과도 효과)** | 보류 → **드롭 확정** (2026-08-20) | 아래 근거 2건. 구 §3 우선순위 3 |

→ §4-1(RayFlare 연동 시점)은 이 결정으로 **해소됐다.**

#### Capacitive effects 드롭 근거 (2건)

**1. 따라갈 대상이 애초에 없다 — 매뉴얼 원문 대조 (2026-08-19, KIST PC).**
이 항목의 유일한 전제는 *"Griddler PRO가 구현한 용량 모델을 따라간다"* 였고,
원문 대조에서 그 전제가 무너졌다.

| 확인 | 원문 |
|---|---|
| **§1.2 등가회로에 용량 소자가 없다** | 소자는 n=1 다이오드 · n=2 다이오드 · 광생성 전류원 · shunt 컨덕턴스가 전부. `capacitance`가 소자를 가리켜 쓰인 곳 **0건** |
| **§6.6 수식 0줄** | 문단 1 + 명령어 2(`SETSWEEPRATE`·`SETSWEEPTIMING`) + 그림 1이 전부. 용량 모델·시간 적분 방식·파라미터화가 **하나도 기술되어 있지 않다.** 지목하는 원인도 용량이 아니라 *"high lifetime → 정상상태 도달 시간"* |
| **§6.2가 정상상태 전용임을 명시** | *"the Griddler solver is **inherently for steady state situations**"* — AC 응답도 소자가 아니라 **복소 벌크 수명**으로 근사한다. FEM 네트워크에 소자가 추가되지 않는다 |

→ 따라갈 참조 구현이 없으므로 **모델을 새로 정의해야** 하고, 그러면 PRO와 대조
가능한 범위를 벗어난다. 상세는 계획서 §1-0 판정표(#6~#12)와 §6-0.

**2. 물리적으로 필요하지 않다 — 박사님 판단 (2026-08-20 랩미팅).**
**실리콘에서 커패시티브 효과는 거의 무시할 수준이어서 구현하지 않아도 된다.**
1번이 "따라갈 것이 없다"는 판정이라면, 2번은 **없어도 된다**는 판정이다.
둘 중 하나만으로도 착수 근거가 사라지지만, 두 근거의 성격이 다르므로 함께 남긴다.

**정리 방침**

- 계획서 `docs/superpowers/plans/2026-08-19-capacitive-effects.md` — **삭제하지 않는다.**
  드롭 판정의 근거가 그 안에 있다. 머리에 드롭 확정 표시를 달아 두었다.
- `tests/test_transient_layout.py` **47건 — 유지한다.** 시간 항이 아니라 **선형
  시스템의 모양**(미지 벡터 레이아웃 · 희소성 패턴 · 대각 슬롯의 구조적 존재)을
  고정하는 테스트라 드롭과 무관하게 유효하다. 지우면 그 불변식만 조용히 풀린다.
- `docs/registration_material.md` §7의 **"정상상태 전용"** 한계 기술 — 이제
  **의도된 범위**임을 명시했다(미구현 격차가 아니다).
...
>
> **소재 (2026-08-19 확인)**: KIST PC `~/Desktop/이승훈/Griddler_and_PRO_manual.pdf`
> (v7.0 2023-09-15, 123쪽). **집 데스크톱·맥북에는 없다** — 그쪽에서 매뉴얼 근거가
> 필요하면 `docs/griddler_feature_map.md`가 유일한 출처다. 그래서 §1.2 등가회로 ·
> §4.3 계산기 · §6.2/§6.6 시간 의존 항목의 원문 요약을 그 문서에 옮겨 두었다.
